...
export function SearchBar(){
    ...
    useEffect(() => {
        ...
        if (inputValue === "") {
            setFilteredProducts(products)
            setIsModalOpened(products.length > 0)
        } else {
            ...
            setIsModalOpened(productsTemp.length > 0)
    ...
    return(
        <div ref={modalContainerRef}>
            {/* event.stopPropagation() - зупиняє всплитіє */}
             <input className="input" type="text" placeholder="Пошук продуктів..." onChange={(event) => {setInputValue(event.target.value)}} onFocus={inputOnFocus} onClick={(event) => {event.stopPropagation()}}/>
             { isModalOpen === true 
                    ...
                    container={(modalContainerRef.current) ? modalContainerRef.current : undefined}>
                            <div className='SearchBarItems'>
                                {filteredProducts.map((product, index)=>{
                                    return (
                                        <div className='SearchBarItem' key={index}>
                                        ...
                                        <Link to={"/product/"+product.id} className="SearchItemTitle" onClick={(event) => {setIsModalOpened(false)}}>{product.title}</Link>
                                        </div>
                                    )
                                })}
                            </div>
