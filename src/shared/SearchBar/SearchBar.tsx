...
export function SearchBar(){
    ...
    var inputSearchBar = useRef< HTMLInputElement | null >(null)
    const modalContainerRef = useRef< HTMLDivElement | null >(null)


    function updSearchedProducts () {
        if (inputSearchBar.current?.value.length != 0) { 
            setSearchedProducts(
                products.filter((product) => {
                    return product.title.toLowerCase().includes(String(inputSearchBar.current?.value))
                })
            )
        } else {
            setSearchedProducts(products)
        }
        console.log(inputSearchBar.current?.value)
    }
    ...
    return(
        <div ref={modalContainerRef}>
            <input ref={inputSearchBar} onChange={updSearchedProducts} className="input" type="text" placeholder="Пошук продуктів..." onFocus={inputOnFocus} onClick={(event) => {event.stopPropagation()}}/>
            { 
                isModalOpen === true
                ? 
                <Modal 
