...
export function SearchBar(){
    ...
    return(
        <div ref={modalContainerRef}>
            <input className="input" type="text" value={inputValue} onChange={inputChange} placeholder="Пошук продуктів..." onFocus={inputOnFocus} onClick={(event) => {event.stopPropagation()}}/>
            { isModalOpen === true ? 
                    ...
                    <div className="SearchBarItems">
                        ...
                        filteredProducts.length > 0 ? (
                            filteredProducts.map((product) => <SearchBarItems product={product} />)
                        ) : ( <p>Нет товаров, соответствующих запросу.</p>)
                        ) : (
                        // Если поле ввода пустое, показываеться все товары
                        products.map((product) => <SearchBarItems product={product} />)
                        )}
                    </div>
                    </Modal>
                    : 
